...
python scripts/ingest.py [PATH] [--title T] [--author A] [--category C] \
    [--description D] [--tags "a,b,c"] [--meta KEY=VALUE ...]   # ingestion (default ./data)
...
- `IVectorStore` → `ChromaVectorStore` (local, default) / `AzureSearchVectorStore` (Azure AI Search) — selected via `stores/factory.py:create_vector_store`
